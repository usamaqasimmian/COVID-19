...
async function Third() {
    ...
    for (let i = 0; i < covid19.Countries.length; i++) {
        var countryCode = await fetch("https://restcountries.com/v3.1/alpha/" + covid19.Countries[i].CountryCode);
    ...
    var options = {
        colorAxis: { minValue: 0, maxValue: 5000000, colors: ["#E5D1E3", "#871F78"] },
    };
...
async function getConfirmed() {
    ...
    if (myDataArray.length == 1) {
        ...
        var countryCode = await fetch("https://restcountries.com/v3.1/name/" + country);
        var countryData = await countryCode.json();
        var population;
        if (country == "India") {
            code = countryData[1].cca2;
